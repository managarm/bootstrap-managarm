...
def extract_debug(elf_path, *, arch, build_id, destdir, use_binutils):
    if len(build_id) < 32:
    ...
    if use_binutils:
        command = f"{arch}-managarm-objcopy"
    args = [
        command,
        "--only-keep-debug",
        elf_path,
...
def strip_debug(elf_path, *, arch, use_binutils):
    command = "llvm-strip"
    if use_binutils:
        command = f"{arch}-managarm-strip"
    args = [
        command,
        "--strip-debug",
        elf_path,
...
def main():
    ...
    parser.add_argument("--binutils", action="store_true")
