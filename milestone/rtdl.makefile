...
$f_CONFIGURE += --protoc= --host-cxx=g++ --host-cppflags= --host-ldflags=
...
configure-$f: | $(call milestone_tag,cross-gcc)
...
install-$f: | $(call milestone_tag,configure-$f)
	cd $B/$g && $($f_RUNPKG) make all-ld-init/linker && $($f_RUNPKG) install-ld-init/linker
